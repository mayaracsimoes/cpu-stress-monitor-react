...
function setupIpcHandlers() {
  ...
  ipcMain.handle("get-detailed-system-info", async () => {
    try {
      const [osInfo2, system2, memLayout2, graphics2, diskLayout2] = await Promise.all([
        si.osInfo(),
        // Versão do Windows/Linux
        si.system(),
        // Fabricante e Modelo da Placa-mãe
        si.memLayout(),
        // Pentes de memória (RAM) individuais
        si.graphics(),
        // Placas de vídeo
        si.diskLayout()
        // SSDs e HDs físicos
  ...
  monitorInterval = setInterval(async () => {
    try {
      ...
      const data = {
        ...
        coresLoad: Array.isArray(cpuLoad.cpus) ? cpuLoad.cpus.map((core) => core.load.toFixed(1)) : [],
